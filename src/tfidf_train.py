...
def main():
    ...
    np.save(os.path.join(MODELS_DIR, "train_tfidf_features.npy"), X_train.toarray())
    np.save(os.path.join(MODELS_DIR, "test_tfidf_features.npy"), X_test.toarray())
    ...
    joblib.dump(lr, os.path.join(MODELS_DIR, "best_model.pkl"))

    print("\nMaking predictions on test data...")
    predictions = lr.predict(X_test)

    output = pd.DataFrame(data={"id": test["id"], "sentiment": predictions})
    output.to_csv(os.path.join(SUBMISSION_DIR, "TF-IDF_model.csv"), index=False, quoting=3)

    print(f"Done! Submission file saved to submission/TF-IDF_model.csv")
